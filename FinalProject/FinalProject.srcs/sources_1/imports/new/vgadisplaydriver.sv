...
module vgadisplaydriver#(
   ...
   wire [$clog2(charCount*charDim*charDim)-1:0] bmem_addr;      // starting address of current character color data 
   wire [11:0] bmem_color;
   
   assign screenAddr = j+(k*ncols);                        // index of character code in screen memory 
   assign bmem_addr = {charCode,offsetY,offsetX};          // index of color value in bitmap memory 
   ...
   bitmapmem #(
    ...
    .bitmapAddr(bmem_addr), 
    .colorValue(bmem_color)
   ); 

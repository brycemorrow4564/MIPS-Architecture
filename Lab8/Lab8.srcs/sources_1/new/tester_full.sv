...
// with full_dmem.mem.
//
// The test program has 59 instructions after compilation.  So, your instruction
// memory is set to size 64 by the tester.  For data, although two integers are
// stored in data memory, a stack is also needed for implementing procedure calls.
// In this test, data memory is also set to size 64, which leaves ample room for
// the stack.
//
...
// inputs, and no debug outputs.  Instead, the internal signals are "pulled out"
...
module mips_tester_full;
    ...
    top #(
        .Dbits(32),                        // word size for the processor
        .Nreg(32),                         // number of registers
        .imem_size(64),                    // imem size; the "full_imem.mem" test has 59 instructions
